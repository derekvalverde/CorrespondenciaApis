...
{
    public class ApiMaterialVentaStokService:IApiMaterialVentaStokService
    {
        private readonly AppSettings _appSettings;
        private readonly ApplicationDbContext _context;
        public ApiMaterialVentaStokService(IOptions<AppSettings> appSettings, ApplicationDbContext context)
        {
            _appSettings = appSettings.Value;
            _context = context;
        }
        public List<clsMaterialVentaStockResponse> obtenerMateriales(int uclId, string like, string cliCodigo, string ageOficina, int usuId, string aplicacion,  int permiso)
        {
            //
            //Aplicacion=sistema de Intrix o PP Pedidos personales
            //aplicacion='PC' entonces es PedidosCliente Intrix5
            //aplicacion='PP' entonces es PedidosPersonal Inti_PP
            ...
            if(aplicacion=="PC")
            {
                cmd.CommandText = "Api_MaterialVentaStock";
            ...
            if (aplicacion == "PP")
            {
                cmd.CommandText = "Api_MaterialVentaPersonalPrincipio";
                cmd.Parameters.Add("@matCodigo", System.Data.SqlDbType.VarChar, 18).Value = like;
            ...
            var buscador = new List<clsMaterialVentaStockResponse>();
            ...
            while (reader.Read())
            {
                buscador.Add(MapToValue(reader,aplicacion));
            }
            conn.Close();
        ...
        private clsMaterialVentaStockResponse MapToValue(SqlDataReader reader, string aplicacion)
        {
